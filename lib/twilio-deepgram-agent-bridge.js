...
function makeIntakeState() {
  return {
    ...
    transcripts: [], completeLogged: false, _recentUtterances: []
  };
}
...
const extractEmail = t => { const m = (t||'').match(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/i); return m ? m[0] : null; };
const extractClientType = t => { const s = (t||'').toLowerCase(); if (/\b(existing|current|already.*client)\b/.test(s)) return 'existing'; if (/\b(new|potential|not.*client|accident|injury|case)\b/.test(s)) return 'new'; return null; };
const extractFullName = t => {
  if (!t) return null;
  let m = t.match(/\b(?:my name is|this is|i am|i'm)\s+([a-z][a-z\s\.'-]{2,})$/i);
  if (m) { const cand = m[1].replace(/\s+/g, ' ').trim(); if (cand.split(/\s+/).length >= 2) return toTitle(cand); }
  m = t.match(/\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+){1,2})\b/);
  return m ? m[1] : null;
};
const extractDate = t => {
  if (!t) return null;
  const m = t.match(/\b(?:jan|feb|mar|apr|may|jun|jul|aug|sept?|oct|nov|dec)[a-z]*\s+\d{1,2}(?:st|nd|rd|th)?(?:,\s*\d{4})?/i)
...
function isComplete(intake) { return !!(intake.client_type && intake.full_name && (intake.phone || intake.email) && intake.incident && intake.date && intake.location); }
function intakeSnapshot(intake) { const { client_type, full_name, phone, email, incident, date, location } = intake; return { client_type, full_name, phone, email, incident, date, location, complete: isComplete(intake) }; }
function updateIntakeFromUserText(intake, text) {
  if (SHADOW_ENABLED) {
    ...
    recent.push(incoming); if (recent.length > 25) recent.shift();
    if (SHADOW_LOG_MODE === 'verbose') { log('debug', 'intake_shadow_seen', { text: incoming }); }
    intake.transcripts.push(incoming);
    let changed = false;
    const maybe = (label, val) => { if (val && !intake[label]) { intake[label] = val; changed = true; if (SHADOW_LOG_MODE !== 'summary') log('info', 'intake_field', { field: label, value: intake[label] }); } };
...
// Lazy load finalize-intake to avoid boot failures if OPENAI_API_KEY is missing
let _finalizeLoaded = false;
let _finalizeFn = null;
function getFinalizeFn() {
  if (_finalizeLoaded) return _finalizeFn;
  _finalizeLoaded = true;
  try {
    // eslint-disable-next-line global-require
    const { finalizeIntake } = require('./finalize-intake');
    _finalizeFn = finalizeIntake;
  } catch (e) {
    log('warn', 'finalize_intake_unavailable', { err: e?.message || String(e) });
    _finalizeFn = null;
  }
  return _finalizeFn;
}
...
function setupBidiBridge(server, { route = '/twilio-bidi' } = {}) {
  ...
  wss.on('connection', (twilioWS, req) => {
    ...
    let llmModel = (process.env.LLM_MODEL || 'gpt-4o-mini').trim();
    ...
    agentWS.on('message', (data) => {
      ...
      if (evt) {
        switch (evt.type) {
          case 'Welcome': log('info', 'dg_evt_welcome'); break;
          case 'SettingsApplied':
            settingsApplied = true;
            log('info', 'dg_evt_settings_applied');
            if (preRollChunks.length) {
              try { for (const c of preRollChunks) if (agentWS.readyState === WebSocket.OPEN) agentWS.send(c); }
          ...
          case 'UserStartedSpeaking':
            ...
            break;
          case 'AgentWarning': log('warn', 'dg_evt_warning', evt); break;
          case 'AgentError':
          case 'Error': log('error', 'dg_evt_error', evt); break;
          default: log('debug', 'dg_evt_other', evt);
        }
        return;
      }
    ...
    agentWS.on('close', (code, reason) => {
      ...
      if (SHADOW_ENABLED) log('info', 'intake_final', intakeSnapshot(intake));

      // Post-call normalization → structured JSON (lazy require to avoid boot failure)
      (async () => {
        try {
          const finalizeIntake = getFinalizeFn();
          if (typeof finalizeIntake === 'function') {
            ...
            log('warn', 'intake_finalize_skipped', { reason: 'finalize-intake not available' });
          }
        } catch (e) {
